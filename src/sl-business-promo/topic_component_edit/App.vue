...
export default {
  ...
  methods: {
    ...
    // 获取原来有得数据
    getDetail() {
      seeFetch('sl-business-promo/topicComponentEdit/getComponent', {
    ...
    delMenu(item) {
      this.$confirm('确定删除此菜单吗？', '提示', {
        ...
        seeFetch('sl-business-promo/topicComponentEdit/addTag', {
          id: item.id,
          topicId: this.topicId,
          status: -1,
        }).then(res => {
    ...
    saveBindCom() {
      ...
      seeFetch(
        'sl-business-promo/topicComponentEdit/addComponentTag',
        params
      ).then(res => {
        if (!res.errorCode) {
          this.bindVisible = !1;
    ...
    delRow(scope, comIndex) {
      ...
      this.$confirm('确定删除当前行吗？', '提示', {
        ...
        // seeFetch('sl-business-promo/topicComponentEdit/addComponent', params).then(res => {
        //   if (!res.errorCode) {
        //     Notification({
        //       type: 'success',
        //       title: '提示',
    ...
    saveComponent(comIndex) {
      ...
      const params = { id, name, type, topicId: this.topicId, dataList };
      seeFetch(
        'sl-business-promo/topicComponentEdit/addComponent',
        params
      ).then(res => {
    ...
    delComponent(comIndex) {
      this.$confirm('确定删除当前组件吗？', '提示', {
        ...
        seeFetch('sl-business-promo/topicComponentEdit/delComponent', {
          id: comId,
        }).then(res => {
          if (!res.errorCode) {
