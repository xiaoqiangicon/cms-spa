  ...
  methods: {
    init() {
      ...
      if (id) {
        this.topicId = id;
        this.getDetail();
        // this.form = JSON.parse(window.sessionStorage['sl-business-promo/topic/item']);
      }
    },
    initSortable() {
      // const self = this;
    ...
    getDetail() {
      ...
      seeFetch('sl-business-promo/topicEdit/getDetail', { topicId }).then(
        res => {
          if (!res.success) {
            Notification({
              type: 'error',
    ...
    handleClickSave() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          // 数据上传
          seeFetch('sl-business-promo/topicEdit/update', {
            id: this.topicId,
            title,
            introduce,
            isShowWish,
            ...
            this.$router.replace(
              `/sl-business-promo/topicEditComponent/${res.data.topicId}/${this.form.title}`
            );
          });
        }
      });
