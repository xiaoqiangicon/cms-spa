...
import { shiJingAddProps } from './data';

const computedProps = {};

shiJingAddProps.forEach(({ name, full }) => {
  if (full) {
    computedProps[name] = {
      get() {
        return this.$store.state.financeIncome.shiJingAdd[name];
      },
      set(value) {
        const key = `financeIncome/shiJingAdd/update${name
          .slice(0, 1)
...
export default {
  ...
  methods: {
    clickCancel() {
      this.$store.commit(`financeIncome/shiJingAdd/updateVisible`, !1);
    },
    clickOk() {
      ...
      this.saving = !0;
      seeFetch('finance/income/updateShiJing', {
        id: this.updateId,
        serviceCharge,
        takeEffectTime,
      }).then(res => {
