...
addProps.forEach(({ name, full }) => {
  if (full) {
    computedProps[name] = {
      get() {
        return this.$store.state.financePending.add[name];
      },
      set(value) {
        const key = `financePending/update${name
          .slice(0, 1)
...
export default {
  ...
  methods: {
    clickCancel() {
      this.$store.commit(`financePending/updateVisible`, !1);
    },
    clickNo() {
      this.$prompt('请输入收据有什么问题', '提示', {
        ...
        this.saving = !0;
        seeFetch('finance/income/makeQuestion', {
          id: this.updateId,
          reason: value,
        }).then(res => {
          this.saving = !1;
    ...
    clickOk() {
      this.$confirm(
        ...
        this.saving = !0;
        seeFetch('finance/income/makeDone', { id: this.updateId }).then(res => {
          this.saving = !1;
