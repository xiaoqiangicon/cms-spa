...
import { ziYingUseProps } from './data';

const computedProps = {};

ziYingUseProps.forEach(({ name, full }) => {
  if (full) {
    computedProps[name] = {
      get() {
        return this.$store.state.financeIncome.ziYingUse[name];
      },
      set(value) {
...
export default {
  ...
  methods: {
    clickCancel() {
      this.$store.commit(`financeIncome/ziYingUse/updateVisible`, !1);
    },
    clickOk() {
      const { foShiId, desc } = this;
      ...
      this.saving = !0;
      seeFetch('finance/income/addZiYingUse', {
        foShiId,
        amount,
        desc,
      }).then(res => {
